...
<h3 class="w-full inline-flex justify-between text-3xl mb-4">
    <span class="text-gray-800 dark:text-gray-300 uppercase underline">{{ $school->name }} Blogs</span>
</h3>
<section class="row px-2">
    ...
    <div class="col-sm-12 col-md-8 rounded-lg bg-transparent dark:bg-transparent">
        <h3 class="w-full inline-flex justify-between text-3xl mb-4">
            <span class="text-gray-800 dark:text-gray-300 uppercase underline">View Blogs.</span>
        </h3>

        {{-- conditional on blogs count  --}}
        @if ($school->blogs->count() == 0)
            <h3 class="w-full inline-flex justify-center text-6xl mb-4">
                <span class="text-rose-800 dark:text-rose-500 uppercase underline underline-offset-[10px] decoration-rose-600 dark:decoration-rose-600">(0) Blogs!</span>
            </h3>

            <hr class="mx-auto flex border-rose-800 dark:border-rose-300 w-[75%] text-center my-4 border-b-[3px] rounded-md">
        @else
            {{-- carousel  --}}
            @include('partials.carousels.blog-carousels.blog-carousels')
            {{-- end carousel  --}}
        ...
        {{-- school blogs tables  --}}
        @include('partials.tables.blogs-tables.blogs-tables')
        {{-- end school blogs tables  --}}

        <hr class="mx-auto flex border-cyan-800 dark:border-cyan-300 w-[75%] text-center my-4 border-b-[3px] rounded-md">

        <h3 class="w-full inline-flex justify-between text-3xl my-2">
            <span class="text-gray-800 dark:text-gray-300 uppercase underline">Recent Blogs.</span>
        </h3>

        {{-- cards  --}}
        <section class="my-4 w-full grid grid-cols-3 space-x-2 space-y-2">
            @foreach ($school->latestBlogs as $blog)
                <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <a href="/blogs/{{ $blog->id }}">
                        <img class="rounded-t-lg" src="{{ asset($blog->thumbnail_path) }}" alt="{{ $blog->blog_title }} by {{ $school->name }} on goshule.com" />
                    </a>
                    <div class="p-2">
                        <a href="/blogs/{{ $blog->id }}">
    ...
    <div class="col-sm-12 col-md-4">
        @include('partials.forms.blogs.add-blogs')
    </div>
</section>